...
import { GsapFadeUp, GsapSlideIn, GsapScale, GsapStagger } from "@/components/gsap-reveal";
import {
  Mic, ArrowRight, Globe, Video, Mail,
  ChevronDown, Star, Zap, Brain, Clock,
  CheckCircle2, Play, X, MessageSquare,
  MousePointer2, Keyboard, Eye, Search,
} from "lucide-react";
...
const HOW_STEPS = [
  {
    ...
    image: voiceToAction,
    tag: "Voice Input",
    detail: "Works with any microphone. Understands natural speech, accents, and context.",
  },
  {
    number: "02",
    ...
    image: demoBrowser,
    tag: "Browser Control",
    detail: "Clicks, scrolls, navigates, and reads live web content in real time.",
  },
  {
    number: "03",
...
const CANT_DO = [
  { icon: <MousePointer2 className="w-5 h-5" />, title: "Click anything on screen", desc: "Generic AI can only chat. BrowseAI actually clicks, scrolls, and navigates like a real human." },
  { icon: <Globe className="w-5 h-5" />, title: "Browse live websites", desc: "ChatGPT's knowledge is frozen. BrowseAI visits real websites right now and reads live content." },
  { icon: <Keyboard className="w-5 h-5" />, title: "Fill forms & create accounts", desc: "No AI chatbot can register you on websites. BrowseAI types, submits, and completes forms for you." },
  { icon: <Mail className="w-5 h-5" />, title: "Send real emails", desc: "Others draft text. BrowseAI opens Gmail, writes, and sends your email — while you do something else." },
  { icon: <Eye className="w-5 h-5" />, title: "See your screen in real time", desc: "Generic AI is blind to your browser. BrowseAI watches your screen and reacts to what it sees." },
  { icon: <Video className="w-5 h-5" />, title: "Find YouTube clips", desc: "No chatbot can play a video. BrowseAI navigates to exact timestamps and summarizes what it watched." },
];

const TESTIMONIALS = [
  { name: "Sarah Chen", role: "Product Manager", text: "I researched an entire market in 20 minutes. BrowseAI navigated, read, and summarized everything while I had my coffee.", stars: 5, avatar: "SC" },
  { name: "Marcus Rodriguez", role: "Freelance Developer", text: "I said 'write a follow-up email' and it opened Gmail and drafted the perfect reply. Done.", stars: 5, avatar: "MR" },
  { name: "Priya Nair", role: "Content Creator", text: "Finding clips used to take hours. Now I describe the moment and BrowseAI finds the exact YouTube timestamp.", stars: 5, avatar: "PN" },
];
...
export default function Home() {
  ...
  useEffect(() => {
    const t = setInterval(() => setCommandIdx(i => (i + 1) % COMMANDS.length), 3200);
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(".hero-badge", { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, ease: "power3.out" })
        ...
        .fromTo(".hero-word", { opacity: 0, y: 40, skewY: 3 }, { opacity: 1, y: 0, skewY: 0, duration: 0.7, stagger: 0.07, ease: "power3.out" }, "-=0.3")
        // Sub copy
        .fromTo(".hero-sub", { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, ease: "power3.out" }, "-=0.4")
        // CTAs
        .fromTo(".hero-cta", { opacity: 0, y: 16 }, { opacity: 1, y: 0, duration: 0.5, stagger: 0.1, ease: "power3.out" }, "-=0.3")
        // Command bar
        .fromTo(".hero-bar", { opacity: 0, y: 20, scale: 0.97 }, { opacity: 1, y: 0, scale: 1, duration: 0.6, ease: "back.out(1.3)" }, "-=0.2")
        // Trust badges
    ...
    <div className="bg-background text-foreground overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-14 py-4 border-b border-white/[0.06] backdrop-blur-2xl bg-background/80">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-lg tracking-tight text-white">BrowseAI</span>
        </div>
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map(l => (
            <button key={l} className="text-sm text-white/45 hover:text-white transition-colors font-medium">{l}</button>
          ))}
        </div>
        <div className="flex items-center gap-3">
          <Link href="/sign-in">
            <button className="text-sm font-medium text-white/55 hover:text-white transition-colors">Log in</button>
          </Link>
          <Link href="/sign-up">
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center pt-20 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute inset-0 opacity-[0.022]" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,1) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,1) 1px, transparent 1px)", backgroundSize: "65px 65px" }} />
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-14 w-full grid lg:grid-cols-[1fr_480px] gap-8 items-center py-16 lg:py-24">
          ...
          <div className="relative z-10 flex flex-col">
            <div className="hero-badge inline-flex items-center gap-2 self-start px-3.5 py-1.5 rounded-full border border-primary/25 bg-primary/8 text-primary mb-7 opacity-0">
              ...
              <span className="text-xs font-semibold tracking-widest uppercase">AI Agent Online — v2.0</span>
            </div>

            <h1 ref={headlineRef} className="text-5xl md:text-6xl lg:text-[72px] font-black tracking-tight leading-[1.03] mb-6 overflow-hidden">
              {["Your browser,", "commanded", "by voice."].map((line, li) => (
                <div key={li} className="overflow-hidden">
                  <span className={`hero-word block opacity-0 ${li === 1 ? "text-gradient" : "text-white"}`}>
                    {line}
                  </span>
                </div>
              ))}
            ...
            <p className="hero-sub opacity-0 text-base md:text-lg text-white/45 max-w-lg mb-10 leading-relaxed">
              BrowseAI is your AI co-pilot that watches your screen, navigates the web,
              writes emails, and executes real tasks — through your voice alone.
              No typing. No clicking. Just speak.
            </p>

            <div className="flex flex-wrap items-center gap-4 mb-12">
              <Link href="/sign-up">
                <Button size="lg" className="hero-cta opacity-0 h-12 px-8 rounded-full bg-primary hover:bg-primary/90 text-white font-semibold text-base gap-2 shadow-2xl shadow-primary/30 group">
                  Start for free
            ...
            <div className="hero-bar opacity-0 glass-panel rounded-2xl px-4 py-3.5 flex items-center gap-4 max-w-xl">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] text-white/25 uppercase tracking-widest mb-0.5 font-semibold">Listening</p>
                <AnimatePresence mode="wait">
                  <motion.p key={commandIdx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.35 }} className="text-white/70 text-sm font-mono truncate">
                    "{COMMANDS[commandIdx]}"
                  </motion.p>
                </AnimatePresence>
              </div>
              <div className="flex items-end gap-[2px] h-5 shrink-0">
                {[0, 1, 2, 3, 4].map(i => (
                  <motion.div key={i} animate={{ scaleY: [0.2, 1, 0.3, 0.8, 0.2] }} transition={{ duration: 1.2, delay: i * 0.12, repeat: Infinity }} className="w-[3px] bg-primary rounded-full" style={{ height: "100%", transformOrigin: "bottom" }} />
                ))}
              </div>
            </div>
            ...
            <div className="hero-trust opacity-0 flex items-center gap-6 mt-8 text-xs text-white/25">
              ...
              <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> Setup in 60s</span>
            </div>
          </div>

          {/* Right: Girl agent (no background) */}
          <div className="relative hidden lg:flex items-end justify-center" style={{ minHeight: 620 }}>
            {/* Glow halo */}
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-80 h-80 bg-primary/25 rounded-full blur-[100px]" />
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-48 h-48 bg-accent/20 rounded-full blur-[60px]" />

            {/* Floating status cards */}
            <motion.div animate={{ y: [-5, 5, -5] }} transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut" }}
              className="hero-float-card absolute top-10 -left-6 glass-panel px-4 py-3 rounded-2xl z-20 border-white/10 shadow-xl">
              <p className="text-xs text-white/40 mb-0.5">Task completed</p>
              <p className="text-sm font-bold text-white">Email sent to client</p>
            ...
            <motion.div animate={{ y: [6, -6, 6] }} transition={{ duration: 3.8, repeat: Infinity, ease: "easeInOut" }}
              className="hero-float-card absolute top-1/3 -right-4 glass-panel px-4 py-3 rounded-2xl z-20 border-white/10 shadow-xl">
              <p className="text-xs text-white/40 mb-1">Now browsing</p>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                <span className="text-sm font-bold text-white">youtube.com</span>
              </div>
              <p className="text-[10px] text-white/30 mt-1">Searching for clips…</p>
            </motion.div>

            <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
              className="hero-float-card absolute bottom-28 -left-8 glass-panel px-3.5 py-3 rounded-2xl z-20 border-white/10">
              <div className="flex items-center gap-2.5">
                <div className="flex items-end gap-0.5 h-5">
                  {[0, 1, 2, 3].map(i => (
                    <motion.div key={i} animate={{ scaleY: [0.3, 1, 0.3] }} transition={{ duration: 0.9, delay: i * 0.15, repeat: Infinity }}
                      className="w-1 bg-primary/80 rounded-full h-full" style={{ transformOrigin: "bottom" }} />
                  ))}
                </div>
                <span className="text-xs text-white/55 font-medium">Speaking results…</span>
              </div>
            </motion.div>

            <motion.div animate={{ y: [4, -4, 4] }} transition={{ duration: 3.2, repeat: Infinity, ease: "easeInOut" }}
              className="hero-float-card absolute top-6 right-2 glass-panel px-3 py-2 rounded-xl z-20 border-white/10">
              <div className="flex items-center gap-2">
            ...
            {/* Girl image */}
            <div ref={girlRef} className="relative z-10 flex items-end justify-center" style={{ height: 600 }}>
              <img
                src={girlAgent}
                alt="BrowseAI AI Agent"
                className="h-full w-auto object-contain object-bottom select-none"
                style={{ filter: "drop-shadow(0 0 60px rgba(139,92,246,0.55)) drop-shadow(0 20px 80px rgba(99,60,180,0.35))" }}
                draggable={false}
              />
              {/* Ground glow line */}
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <GsapFadeUp className="text-center mb-20">
            ...
            <h2 className="text-4xl md:text-6xl font-black text-white tracking-tight leading-tight">
              From voice command<br />
              <span className="text-gradient">to completed task</span>
            </h2>
            <p className="text-white/35 text-base max-w-2xl mx-auto mt-4">
              Watch BrowseAI handle complex multi-step tasks from a single spoken sentence.
              No prompts. No templates. Just natural language.
            </p>
          </GsapFadeUp>
          ...
          <div className="flex flex-col gap-24">
            {HOW_STEPS.map((step, i) => (
              ...
              >
                {/* Image side */}
                <GsapSlideIn direction={i % 2 === 0 ? "left" : "right"} className="[direction:ltr]">
                  <div className="relative rounded-3xl overflow-hidden border border-white/10 shadow-2xl shadow-black/50 group">
                    ...
                    <div className="absolute bottom-5 left-5 z-20 flex items-center gap-3">
                      <div className="flex items-center gap-2 glass-panel px-3 py-2 rounded-xl border-white/10">
                        <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                        <span className="text-xs font-semibold text-white">{step.tag}</span>
                      </div>
                    </div>
                ...
                {/* Copy side */}
                <GsapSlideIn direction={i % 2 === 0 ? "right" : "left"} delay={0.1} className="[direction:ltr] flex flex-col">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="w-10 h-10 rounded-xl bg-primary/15 text-primary flex items-center justify-center shrink-0">
                      {step.icon}
                    </div>
                    <span className="text-xs font-semibold uppercase tracking-widest text-primary">Step {step.number}</span>
                  </div>
                  ...
                  {i < HOW_STEPS.length - 1 && (
                    <div className="flex items-center gap-2 mt-8 text-white/20 text-xs">
                      <div className="flex gap-1">
                        {[0, 1, 2].map(d => (
                          <div key={d} className="w-1 h-1 rounded-full bg-white/20" style={{ opacity: 1 - d * 0.25 }} />
                        ))}
                      </div>
                      <span>then →</span>
                    </div>
          ...
          <GsapScale className="mt-24" delay={0.05}>
            <div className="relative rounded-3xl overflow-hidden border border-white/10 shadow-2xl shadow-black/50">
              ...
              <img src={demoInterface} alt="BrowseAI full interface demo" className="w-full object-cover" />
              <div className="absolute inset-0 z-20 flex items-end p-8 md:p-12">
                <div className="max-w-lg">
                  <p className="text-xs font-semibold text-primary uppercase tracking-widest mb-2">Live dashboard</p>
                  <h3 className="text-2xl md:text-3xl font-black text-white mb-3">Watch every action in real time</h3>
                  <p className="text-white/45 text-sm leading-relaxed">
                    See exactly what BrowseAI is doing — tabs opening, pages loading, text being typed — all narrated in your ear.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="py-24 px-4 border-t border-white/[0.06] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <GsapFadeUp className="text-center mb-16">
            <span className="text-xs font-semibold uppercase tracking-widest text-primary mb-4 block">The real difference</span>
            ...
            <p className="text-white/35 max-w-xl mx-auto text-base">
              ChatGPT, Claude, Gemini — they're all just text boxes. BrowseAI has hands.
            </p>
          </GsapFadeUp>

          <GsapStagger className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {CANT_DO.map(item => (
              <div key={item.title} className="glass-panel rounded-2xl p-6 group hover:border-primary/20 transition-all duration-300 relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 mb-5">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-500/10 border border-red-500/20">
                    ...
                    <span className="text-xs text-red-400 font-semibold">Generic AI: Cannot</span>
                  </div>
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 border border-primary/20">
                    <CheckCircle2 className="w-3 h-3 text-primary" />
                    <span className="text-xs text-primary font-semibold">BrowseAI: Yes</span>
                  </div>
                </div>
      ...
      <section className="py-24 px-4 border-t border-white/[0.06]">
        <div className="max-w-6xl mx-auto">
          <GsapFadeUp className="text-center mb-16">
            <span className="text-xs font-semibold uppercase tracking-widest text-primary mb-4 block">Social proof</span>
            <h2 className="text-4xl md:text-5xl font-black text-white tracking-tight">
              People who speak, <span className="text-gradient">don't go back</span>
            </h2>
          </GsapFadeUp>

          <GsapStagger className="grid md:grid-cols-3 gap-6" stagger={0.13}>
            {TESTIMONIALS.map(t => (
              <div key={t.name} className="glass-panel rounded-2xl p-7 flex flex-col gap-5 hover:border-white/15 transition-colors">
        ...
        <GsapScale className="max-w-4xl mx-auto relative z-10">
          <div className="glass-panel rounded-3xl p-12 md:p-16 relative overflow-hidden border border-white/8 text-center">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl md:text-6xl font-black text-white mb-4 tracking-tight">Ready to stop typing?</h2>
              <p className="text-white/35 text-lg mb-10 max-w-xl mx-auto">
                Join thousands of professionals who browse, research, and work 10x faster — with nothing but their voice.
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <Link href="/sign-up">
                  <Button size="lg" className="h-13 px-10 rounded-full bg-primary hover:bg-primary/90 text-white font-bold text-base gap-2 shadow-2xl shadow-primary/30 group">
              ...
              <div className="flex flex-wrap items-center justify-center gap-6 mt-8 text-xs text-white/20">
                <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" /> Free forever plan</span>
                <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" /> No credit card</span>
                <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> Setup in 60 seconds</span>
              </div>
            </div>
          </div>
        </GsapScale>
      ...
      <footer className="border-t border-white/[0.06] bg-white/[0.01]">
        <div className="max-w-7xl mx-auto px-6 md:px-14 py-16">
            ...
            <div className="grid md:grid-cols-4 gap-10 mb-14">
              <div className="md:col-span-2">
                ...
                <p className="text-white/35 text-sm leading-relaxed max-w-xs">
                  The AI co-pilot that navigates the web, executes tasks, and explains results — all through your voice alone.
                </p>
                <div className="flex items-center gap-3 mt-6">
                  {[MessageSquare, Globe].map((Icon, i) => (
                    <div key={i} className="w-8 h-8 rounded-lg bg-white/5 border border-white/8 flex items-center justify-center text-white/40 hover:text-white hover:border-white/20 cursor-pointer transition-all">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-white/30 mb-5">Product</p>
                <div className="flex flex-col gap-3">
                  {["Features", "How it works", "Pricing", "Changelog", "Roadmap"].map(l => (
                    <button key={l} className="text-sm text-white/40 hover:text-white transition-colors text-left">{l}</button>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-white/30 mb-5">Company</p>
                <div className="flex flex-col gap-3">
          ...
          <div className="border-t border-white/[0.06] pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-white/20 text-xs">© 2026 BrowseAI Technologies, Inc. All rights reserved.</p>
